...
- **Viewer** — vertical scroll through the file. Lines wrap instead of panning horizontally (horizontal is the shell's), and text can't pinch-zoom (pinch is lane management) — font size will be a setting. When editing arrives, the caret moves via iOS's spacebar-trackpad (hold the spacebar, steer), so navigation costs no gestures at all.
