...
/// recorded so it can be regenerated. Zero-width characters do NOT need a table: Swift already
/// exposes the general category, and a `Character` is a grapheme cluster, so a combining mark
/// has usually merged into its base before it ever reaches here.
enum TerminalWidth {

    /// East Asian Wide and Fullwidth, as (lowerBound, upperBound) scalar ranges.
    ...
    private static func isWide(_ scalar: Unicode.Scalar) -> Bool {
        ...
        var low = 0, high = wideRanges.count - 1
        while low <= high {
    ...
    ///
    /// The FIRST scalar decides: a grapheme cluster like an emoji with a variation selector or a
    /// ZWJ sequence takes its width from the base character, which is what terminals do (and why
    /// a family emoji is two columns wide, not eight).
    static func columns(of character: Character) -> Int {
        ...
        if first.value < 0x20 || first.value == 0x7F { return 0 }
        ...
        case .nonspacingMark, .enclosingMark, .format:
            ...
            if first.value != 0x00AD { return 0 }
        default:
            break
        }
