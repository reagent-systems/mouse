...
final class AgentSession {
    ...
    /// Where in the terminal's transcript the current sign-in/setup began — the container
    /// shows the program's lines from here when it prints rather than draws.
    private(set) var loginTranscriptStart = 0
    ...
    var authenticated: Bool {
        if agent.setting != nil, AgentSettings.shared.isSet(for: agent) { return true }
        if let marker = agent.configured {
            let file = RuntimeStore.home.appendingPathComponent(marker)
            guard let text = try? String(contentsOf: file, encoding: .utf8) else { return false }
            return agent.configuredWhen.isEmpty || agent.configuredWhen.contains { text.contains($0) }
        }
        return agent.setting == nil
    }
    ...
    func login() async {
        ...
        problem = nil
        await pointAtSharedHome(agent, terminal)
        // The runtime is a download, not an assumption — for a setup as for a turn.
        if agent.runtime == .python, RuntimeStore.installed("python") == nil {
            working = true
        ...
        // The runtime shims (ssl, ctypes, …) the agent's own code walks into on wasi, laid
        // before its setup the way they are laid before a turn.
        if agent.runtime == .python { Pip.layShims(in: Pip.sitePackages) }
        loggingIn = true
        defer { loggingIn = false }
        loginTranscriptStart = terminal.lines.count
        guard terminal.run(command) else { return }
        while terminal.isRunning || terminal.program != nil {
            try? await Task.sleep(for: .milliseconds(200))
        }
        if authenticated {
            messages.append(Message(author: .note, text: "\(agent.loginTitle ?? "sign-in") done"))
            return
        }
        // Not finished: the program's own last words say why — in the chat, where the person
        // is, rather than in a transcript the container no longer shows.
        let said = terminal.lines.suffix(from: min(loginTranscriptStart, terminal.lines.count))
            .filter { $0.kind != .command }.map(\.text).joined(separator: "\n")
            .trimmingCharacters(in: .whitespacesAndNewlines)
        messages.append(Message(author: .note, text: "\(agent.loginTitle ?? "sign-in") did not finish"))
        if !said.isEmpty { messages.append(Message(author: .agent, text: String(said.suffix(1500)))) }
    }
    ...
    private func askEmbedded(_ prompt: String, agent: CodingAgent, terminal: TerminalSession) async {
        ...
        for _ in 0..<6 {
            ...
            // Where to ask is hermes's decision, made by its own provider resolution from its
            // own config, and carried out of the step with the request: base URL, key, the
            // model it chose, and the API shape. Mouse is only the transport.
            let endpoint = out["endpoint"] as? [String: Any] ?? [:]
            let baseURL = endpoint["base_url"] as? String ?? ""
            let apiMode = endpoint["api_mode"] as? String ?? "chat_completions"
            guard apiMode == "chat_completions" || apiMode.isEmpty else {
                problem = "hermes chose a \(apiMode) endpoint; only chat-completions endpoints are spoken here yet"
                return
            }
            guard let api = AgentAPI(baseURL: baseURL, key: endpoint["api_key"] as? String ?? "",
                                     model: args["model"] as? String ?? "") else {
                problem = "hermes has no model endpoint configured — run its setup"
                return
            }
            messages.append(Message(author: .note, text: "llm.complete → \(api.baseURL.host() ?? baseURL)"))
