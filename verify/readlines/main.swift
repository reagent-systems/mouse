...
// a terminal: readline is drivable from a plain stream, so the line rules are fully observable
// without one — which is why never having compared them had no excuse.
...
if theirRows.isEmpty {
    print("READLINE MISMATCH — real node produced no rows:\n\(theirText.prefix(400))\n\(theirErr.prefix(400))")
    exit(1)
}
if elapsed > 30 {
    print("READLINE MISMATCH — the run took \(Int(elapsed))s: a pending AbortSignal.timeout held "
          + "the loop open instead of being unref'd")
    exit(1)
}
if differences.isEmpty, theirRows.count == ourRows.count {
    print("READLINE MATCH — all \(theirRows.count) behaviours identical to real node: where a "
          + "line ends (including a \\r and \\n arriving in separate writes, and a last line "
          + "with no newline at all), async iteration and breaking out of it, question in both "
          + "forms and under an AbortSignal, pause and resume, close, and the cursor escapes")
} else {
    ...
    print("READLINE MISMATCH — \(max(differences.count, abs(theirRows.count - ourRows.count))) of \(theirRows.count) differ")
    exit(1)
}
