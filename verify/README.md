...
That sentence was aspirational until 2026-07-31: five package harnesses
assumed a tree the scratch directory happened to have, `rsalegacy` assumed
an RSA keypair someone had generated by hand, and `tty` assumed two captured
claude-code frames. On a clean checkout all seven failed, so the suite as
committed had never actually been green. Each now creates what it needs, and
the captured frames are checked in (they are 3 KB of bytes that cannot be
regenerated without a credentialed claude-code run). Keys are generated, not
committed.
