...
// node defines structuredClone, workerData, postMessage and environmentData in terms of the
// structured clone algorithm. All four were JSON round-trips, which lose Maps, Sets, Dates,
// undefined, BigInt and cycles WITHOUT reporting anything. One codec now backs them all.
...
if bad == 0 && !ours.isEmpty { print("CLONE CONSUMERS MATCH — all \(want.count), across structuredClone, workerData, postMessage and environmentData") }
else { print("CLONE CONSUMERS DIFFER — \(bad) of \(want.count)")
