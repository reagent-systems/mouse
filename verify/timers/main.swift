...
if theirRows.isEmpty {
    print("TIMER MISMATCH — real node produced no rows:\n\(theirText.prefix(400))\n\(theirErr.prefix(400))")
    exit(1)
}
if differences.isEmpty, theirRows.count == ourRows.count {
    print("TIMER MATCH — all \(theirRows.count) behaviours identical to real node: what a handle "
          + "is and what ref, unref and refresh do to it, how every delay is coerced, the phase "
          + "orderings node actually guarantees, extra arguments, clearing by handle and by id, "
          + "intervals, immediates, and the whole timers/promises surface including AbortSignal")
} else {
    ...
    print("TIMER MISMATCH — \(max(differences.count, abs(theirRows.count - ourRows.count))) of \(theirRows.count) differ")
    exit(1)
}
