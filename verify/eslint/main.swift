...
let packagesRoot = here
if !FileManager.default.fileExists(atPath: packagesRoot.appendingPathComponent("node_modules").path) {
    print("installing eslint dependencies with our own package manager…")
    do { _ = try await PackageManager.install(requirements: ["eslint": "^9.39.5"], into: packagesRoot) }
    catch { print("ESLINT FAILED — install: \(error)"); exit(1) }
}
