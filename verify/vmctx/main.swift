...
// Real vm contexts: a second JSContext in the engine's virtual machine, with the sandbox's keys
// bound as live accessors on the guest global. This is what blocked jest.
...
if bad == 0 && !ours.isEmpty { print("VM CONTEXTS MATCH — all \(want.count) behaviours identical to node") }
else { print("VM CONTEXTS DIFFER — \(bad) of \(want.count)")
