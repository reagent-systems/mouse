...
// The same corpus through real node and through this engine, line for line. The elapsed-time
// guard stays: a web stream that never settles does not fail, it HANGS, and a harness that only
// compared output would wait for it forever.
...
if theirRows.isEmpty {
    print("WEB STREAM MISMATCH — real node produced no rows:\n\(theirText.prefix(400))\n\(theirErr.prefix(400))")
    exit(1)
}
if elapsed > 30 {
    print("WEB STREAM MISMATCH — the run took \(Int(elapsed))s: a pending AbortSignal.timeout held "
          + "the loop open instead of being unref'd")
    exit(1)
}
if differences.isEmpty, theirRows.count == ourRows.count {
    print("WEB STREAM MATCH — all \(theirRows.count) behaviours identical to real node: locking "
          + "and what a released reader may still do, the controller's desiredSize and its "
          + "refusals, reading to completion, errors, cancel reaching the source, tee feeding "
          + "both branches, pipeTo and pipeThrough, and the node bridges in both directions")
} else {
    ...
    print("WEB STREAM MISMATCH — \(max(differences.count, abs(theirRows.count - ourRows.count))) of \(theirRows.count) differ")
    exit(1)
}
