...
// The same corpus through real node and through this engine, line for line. Timings themselves
// cannot match between two runs, so the corpus asserts SHAPES and relationships instead — which
// entries exist, what they contain, and that a measure spanning a real wait is not zero.
...
if theirRows.isEmpty {
    print("PERFORMANCE MISMATCH — real node produced no rows:\n\(theirText.prefix(400))\n\(theirErr.prefix(400))")
    exit(1)
}
if elapsed > 30 {
    print("PERFORMANCE MISMATCH — the run took \(Int(elapsed))s: a pending AbortSignal.timeout held "
          + "the loop open instead of being unref'd")
    exit(1)
}
if differences.isEmpty, theirRows.count == ourRows.count {
    print("PERFORMANCE MATCH — all \(theirRows.count) behaviours identical to real node: what a "
          + "mark and a measure ARE, the entry buffers and clearing them by name, measure from "
          + "options and between marks, ordering, toJSON, and PerformanceObserver before and "
          + "after disconnect. No raw time is compared; the relationships are")
} else {
    ...
    print("PERFORMANCE MISMATCH — \(max(differences.count, abs(theirRows.count - ourRows.count))) of \(theirRows.count) differ")
    exit(1)
}
