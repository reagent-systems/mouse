...
// v8.serialize was JSON.stringify — the same defect swept out of deepStrictEqual, at a third
// site. It is what made jest hang with its cache on: a haste map is mostly Maps and Sets, and
// JSON turns those into {} without complaining, so jest read back a cache that looked valid.
...
for line in want {
    guard let r = line.range(of: ": ") else { print("  node reported a throw: \(line)"); bad += 1; continue }
...
if bad == 0 && !ours.isEmpty { print("STRUCTURED CLONE MATCH — all \(want.count) round-trips faithful, as node's are") }
else { print("STRUCTURED CLONE DIFFERS — \(bad) of \(want.count)")
