...
| **Phase G — finite-field DH, and an audit of every remaining refusal** | The rejection-hook overturn suggested its own follow-up: if one refusal died on an unmeasured assumption, sweep the rest for the same shape. Each surviving refusal rests on a claim about the PLATFORM, so each claim was measured. Three outcomes. **zstd survives** — genuinely absent, now measured rather than read. **The TLS-server refusal's REASON is wrong** — it says a handshake cannot go on a raw socket, but `nw_listener_create`, `sec_identity_create` and `nw_parameters_create_secure_tcp` are all there, so it is a different socket layer plus a certificate, not an impossibility. And the crypto symbols (`cccurve448_make_key_pair`, `ccz_init`) exist but are **corecrypto**, which has no ABI stability and is not for third-party linking — *exists* is not *usable*, a distinction the JSC hook earns and these do not. Then the big one: **finite-field DH and primality were refused as "needs a bignum implementation" — and JavaScriptCore has had native `BigInt` the whole time.** A 2048-bit modular exponentiation costs about 2 ms in the engine's own code, so the bignum was never missing; the code was. `createDiffieHellman`, `getDiffieHellman` (all eight MODP groups, byte-identical primes), `generatePrime` and `checkPrime` all ship, Miller-Rabin rejecting Carmichael numbers, and **real node derives the same 2048-bit shared secret from opposite halves of an exchange with this engine**. Four documented gaps closed at once — the same shape as scrypt, which was also refused as a missing primitive and was also just arithmetic nobody had written |
| **Phase G — driving jest, and the `vm` stub that lied** | The third real-package proof aimed at the thing an IDE most needs after running code: running the project's TESTS. mocha already works; jest is what most projects use, and it stresses module resolution, child processes and `vm` contexts together. It found three defects before stopping: `require.resolve.paths` was missing; the `module` core module had **three** keys where node has forty (a test runner builds its own registry on node's — reading `_extensions` for loadable files, `_nodeModulePaths` to walk the same directories, `_cache` to evict between files — so the gaps stop it at startup rather than degrading it); and a msh-bridged child emitted **`close` before `exit`** while never emitting **`spawn`** at all, which is a HANG for anything waiting on the pair. Then jest stopped for a reason worth stating precisely: **the `vm` module was half real and half a lie** — `createContext` returned the sandbox unchanged, so a feature check concluded vm worked, and `runInContext` was simply absent and threw a bare TypeError from the caller's frame. It now names what is missing: a real context needs a second `JSContext` in the same `JSVirtualMachine` plus a live proxied sandbox. Not a platform limit — unbuilt work, and the next boundary if jest matters. Note `createContext` was left WORKING: refusing it broke webpack, which proved the stub was doing something real |
| **Phase G — real `vm` contexts, and jest runs** | The previous boundary named its own successor and this is it. A contextified sandbox is a **second `JSContext` in the engine's virtual machine** — one `JSVirtualMachine` now backs the engine and every vm context, because JSValues only cross between contexts that share one. The sandbox's keys become live get/set accessors on the guest global, so reads and writes stay live in both directions rather than being copied at the boundary, and globals the script CREATES are synced back (node shows those on the sandbox, and an accessor cannot intercept a property that does not exist yet). Own built-ins, no host globals visible inside, `this === globalThis`, objects and functions crossing both ways, throws propagating to the caller's frame: **18 behaviours identical to node**. And with that, **jest runs — same passes, failures, pending tests and counts as real node**, with test files genuinely executing inside vm contexts (their stack frames say `mouse-vm://`). Two honest notes: jest's haste-map cache path never settles here, so the gate sets `cache: false` and the hang is recorded as a lead rather than papered over; and the harness must live OUTSIDE the tree jest crawls, after a multi-megabyte test binary in the project root sent it walking forever |
| **Phase G — the jest cache hang, and `JSON.stringify` at a THIRD site** | The previous boundary left a measured lead: with jest's haste-map cache enabled the run never settled. The cause was `v8.serialize`, implemented as `JSON.stringify` — **the same defect swept out of `deepStrictEqual` and `util.isDeepStrictEqual` two boundaries earlier, at a third site nobody thought to look at.** A haste map is mostly Maps and Sets; JSON turns those into `{}` and does not complain, so jest wrote a cache, read it back, believed it was warm, and waited forever for information that had been silently discarded. Replaced with a real structured clone: Map, Set, Date, RegExp, Buffer and typed arrays, `undefined` distinct from `null`, BigInt, `NaN`/`Infinity`/`-0`, cycles, and shared references that stay shared rather than being duplicated — 24 round-trips as faithful as node's. jest now runs cold AND warm, and the gate runs it twice precisely so the cache path is exercised. The lesson repeats the one from the DH boundary: when a defect has a SHAPE, the sweep has to cover every site with that shape, and "serialize" did not look like "compare" |
| **Phase G — applying the rule that was just written: NINE sites** | The previous boundary ended by adding a rule to AGENTS.md — *a defect has a shape; grep for the MECHANISM, not the concept* — so the first thing to do was run that grep instead of leaving it as advice. `JSON.stringify`/`JSON.parse` standing in for a structural operation turned up at **six more sites**: `structuredClone` (the global whose entire job is to do this correctly), worker `postMessage` in both directions, `workerData`, `setEnvironmentData`, and the IPC wire between parent and child engines — every one of which node defines in terms of the structured clone algorithm, and every one of which was silently dropping Maps, Sets, Dates, `undefined`, BigInt and cycles. The codec is now hoisted to ONE implementation backing all nine, the way the deep-equal fix backs its three. 15 consumer behaviours plus 24 round-trips gated. The rule earned its place the same day it was written |
| **Phase T — a cell is a COLUMN, and the harness was testing a COPY** | Two findings, one of them uncomfortable. First: `TerminalScreen.put` advanced exactly one column per character, but CJK, Kana, Hangul, fullwidth forms and most emoji occupy **two** — so any TUI drawing a box, a table or an aligned column with non-Latin text came out crooked, the error accumulating along the line. `TerminalWidth.swift` now carries East Asian Width W and F as 103 ranges **generated from Unicode 16.0.0** rather than hand-written, with zero-width taken from Swift's own general-category property; `put` reserves a continuation cell, wraps a wide character whole rather than splitting it at the right edge, attaches a lone combining mark to the grapheme already on screen, and clears the partner when either half of a pair is overwritten. 199 widths agree with Python's `unicodedata`. Second, and the reason the first went unnoticed: **the terminal harnesses were building against COPIES of `TerminalScreen.swift` and `TerminalPrograms.swift` in the scratchpad**, so no change to the shipping terminal had ever been gated. The copies are gone, the source sets point at the real files, and selection is now by what a harness REFERENCES rather than by its name — keying on names is a verification gap wearing the costume of a typo. The Node layer's own byte-vs-character handling was swept at the same time and proved already correct: 25 lines identical to node, now a gate rather than an assumption |
| **Phase T — the alternate screen never came BACK** | Found by trying to test the wide-character work end to end: a Node program drew boxes through the TTY, and the grid was empty afterwards. The grid was right; the alternate screen was wrong. `ESC[?1049h` / `ESC[?1049l` is a **save/restore pair** — the normal screen is set aside, the program gets a cleared one, and on exit the user's work comes back. `enterAlternate` cleared without saving and `leaveAlternate` cleared again instead of restoring, so quitting `less`, `top`, vim or any ink TUI **wiped the terminal** rather than returning the user to what was there. It now saves the grid and the cursor and puts both back, falling back to a clear only when a resize during the program left the saved grid the wrong shape. Gated: the ordinary cycle, the cursor, wide text surviving the round trip with its continuation cell intact, a bare `?1049l` staying a no-op, and a double enter not losing the original behind a second save. A visible bug in a shipping surface, sitting in the middle of a path everything else already exercised — which is what happens when the harness for that surface was testing a copy |
| **Phase G+T — the end-to-end TUI gate, and proving a gate can fail** | The wide-character work was verified at the parser and grid level but not through the whole path, and the previous boundary said so rather than pretending otherwise. It exists now: a Node program on a real TTY enters the alternate screen, draws boxed rows of Latin, CJK and mixed text, and **waits** — which is both what a TUI does and the fix for the timing problem that defeated two earlier attempts (a program that runs to completion restores the screen, so there is nothing left to read; and it exits synchronously, so a poll loop never sees the frame). Assertions run on the live grid, then a keystroke through `stdin` ends it, covering the input half too. The gate was then **deliberately broken** — widths forced back to 1 — and confirmed to go red with exactly the ragged output the bug produced (`日本語   ok`), because a gate that cannot fail is worse than no gate: it reads as evidence. That practice is now a rule in AGENTS.md, earned by two gates this session that were green and meaningless |
| **The suite could not report some failures at all** | Applying the rule written one boundary earlier — *prove a gate can fail* — to the SUITE rather than to a single gate. `verify.sh` grades by scanning a harness's output for a verdict word, and two holes made real failures invisible. The keyword grep was case-INSENSITIVE while the classification was not, so a harness ending *"3 lines differ"* was graded **ok**; and a harness whose failure text used none of the words produced an EMPTY verdict, which counted as neither a pass nor a failure — it simply vanished from the totals. Five harnesses were affected, all written this session. Fixed by normalising case, treating a missing verdict as a failure, and extracting the verdict only from TOP-LEVEL lines — the last of those because the first attempt graded jest on its own `failed=1` detail line, which is the expected result of a suite containing a deliberately failing test. Then the fix was proved the same way the rule demands: the alternate-screen restore was broken on purpose and the suite reported `altscreen FAIL` where it had previously said nothing at all. Totals are unchanged at 68 — the failures were hypothetical until they weren't |
| **Phase G — the coder CLASSES, and a measured path to a TLS server** | Two results. The `audit` diagnostic had been naming `BrotliCompress` and `BrotliDecompress` as missing for some time, and it was right: brotli had the factory functions but no class form, so `new zlib.BrotliCompress()` threw while every zlib equivalent worked — and a constructor in a pipeline is how packages reach for it. Checking that also found `createGzip() instanceof zlib.Gzip` was **false** where node says true, because the factory returned a plain Transform; code branches on that. One shared `__coderClass` now backs all ten coders in both forms, verified to CODE and not merely exist (a brotli round trip through the constructor form in a pipeline). Separately, the TLS-server refusal was pushed as far as it can honestly go without the app: a `SecIdentity` **can** be formed from a PEM key and certificate by adding both to the keychain — no PKCS#12 construction needed — and `sec_identity_create` accepts it, which is the piece `NWListener` wants. What is NOT proven is that this works inside the sandboxed app, where the keychain needs entitlements a scratch binary does not have (the probe reported `errSecMissingEntitlement` on the key add and still resolved an identity, which is exactly the shape of a false positive worth distrusting). The refusal's REASON is now accurate: not "a handshake cannot go on a raw socket" but "the identity step is unverified on device" |
| **Phase G — working the audit's list, now that it has earned it** | The `audit` diagnostic was right about `BrotliCompress`, so the rest of what it names got a proper triage instead of being read as noise. Three groups fell out. The `ZSTD_*` block and `crypto.subtle` are correct refusals. The underscore names — `fs._toUnixTimestamp`, `path._makeLong`, `net._normalizeArgs`, `process._tickCallback` and friends — are node INTERNALS and stay out deliberately. But the rest is documented API that had simply been sitting next to internals and inherited their treatment: **`util.MIMEType` and `util.MIMEParams`** (real classes, with quoted parameter values that survive a reassembly round trip), **`util.parseEnv`** (what `--env-file` is built on), **`util.getCallSites`** (how a logger reports where it was called), **`util.diff`**, and **`events.EventEmitterAsyncResource`** plus its two symbols. `util` went 38→44 of 49 and `events` 17→20 of 20; every zlib gap that remains is zstd. 23 behaviours gated against node, including two divergences worth catching — node returns an EMPTY diff for identical input rather than a run of unchanged entries, and leaves `asyncResource.type` undefined. What is left in `util` is five entries: three internals and the `transferableAbort*` pair, which needs transfer semantics this engine does not have |
| **Phase G — `process`, real CPU time, and the audit becomes a GATE** | Finishing the audit's list. Of the twenty entries it named for `process`, thirteen are underscore internals that stay out; the rest split into three public APIs — `threadCpuUsage`, `finalization`, and the deprecated-but-present `assert` — and two that genuinely cannot work here and now refuse BY NAME: `dlopen` (a `.node` addon is compiled machine code, which iOS will not map) and `execve` (there is no exec, which is precisely why child processes here are engines). Building `threadCpuUsage` exposed `process.cpuUsage()` as a stub returning hardcoded **zeros** — the same shape of lie as the old `vm.createContext`, an API that looks like it works and silently reports nothing. It reads `getrusage` now, and the gate asserts the counter ADVANCES across a busy loop rather than merely having the right keys, because zeros pass a shape check. **And the audit itself is promoted from diagnostic to gate**, per the rule that a settled sweep should become one: every absence is now either a name starting with `_`, a measured refusal on an explicit list, or a failure. Proved by deleting `util.parseEnv` and watching it report `SURFACE MISMATCH — util.parseEnv`. 72 assertions, 5 diagnostics |
| **Phase G — the stdio streams were not streams** | The `stdoutpipe` investigation had been printing `DIFFERS` for a long time, and after the audit turned out to be right about `BrotliCompress`, that was worth reading rather than skipping. It was right too: **`process.stdout instanceof Writable` was false**, and checking the other two showed `stderr` and `stdin` failed the same way — the investigation's own probe only tested one of the three. This is the identical fault the zlib coders had, at the surface every program writes through: `stdout` was a hand-built object literal, so anything branching on `instanceof` took the wrong path. Fixed by attaching the real prototypes, which leaves the write path completely untouched (own properties shadow every inherited method) — the gate compares INTERLEAVED output as well as shape, because a stdout that buffered differently would reorder `console.log` against `process.stdout.write` and that would be far worse than the bug. The prototypes are attached after the module system is up: the stdio objects are built BEFORE the core cache exists, so a `coreRequire('stream')` inside them dies in a temporal dead zone — which a `try/catch` I had written swallowed silently until I made it print. Also matched node in a detail the fix exposed: `isTTY`, `columns` and `rows` are ABSENT when output is not a terminal, not `false`. `stdoutpipe` is promoted from investigation to gate; 74 assertions, 4 diagnostics |
| **Phase G — reading the `shapes` list, and what a bad list is worth** | Third investigation in a row to be worth reading rather than skipping, though this one needed sorting first. Its 123 "missing" properties are mostly node INTERNALS — `readableBuffer`, `hexWrite`, `asciiSlice` — and some entries were outright FALSE: it listed `off` and `removeListener` as missing from `crypto.Hash`, which has both, because it enumerates differently from how the methods are inherited. A list that mixes real findings with internals and false positives is still worth reading; it is just not worth trusting, and the difference is checking each entry rather than the total. Four were real and public: **`process.prependOnceListener`** (the one EventEmitter method missing from `process`), **`fd`/`readable`/`writable` on all three stdio streams** (node reports 1, 2 and 0, and callers check them), and **`Response.formData()`** — the only body reader absent from the fetch set, so a handler reading a posted form got "not a function"; it parses urlencoded AND multipart, taking the boundary from the header. Assigning the flags threw `Attempted to assign to readonly property` because some are already accessors, which is why they are defined rather than assigned |
| **Phase G — fixing the INSTRUMENT, then using it** | The previous boundary found the `shapes` sweep reporting properties as missing that plainly exist, and left it there. Leaving a lying instrument in place means the next reader chases ghosts, so this fixes it. The cause was a `depth < 6` cap on the prototype walk, and the mechanism is the reverse of the obvious guess: node's `crypto.Hash` chain is **seven** links (it carries an extra `LazyTransform` layer) and node's `Readable.prototype` defines `on`/`off`/`addListener`/`removeListener` as OWN properties, while ours inherits them from `EventEmitter` one link further down — so the cap cut OUR chain before `EventEmitter` and reported methods we have as absent. What matters is whether a property is REACHABLE, not which link holds it, so the walk now runs to the end. False positives gone: `crypto.Hash` 8 missing → 4, and the total 123 → 108 with every remaining entry real. Then the list was used: the stream introspection getters it named across SIX rows — `readableAborted`, `readableDidRead`, `writableAborted` — now exist, which is how a consumer tells a truncated body from a whole one. 123 → 92 |
| **Phase G — the remaining property gaps, and three things node does that docs do not say** | Working the (now trustworthy) shapes list down: the server option properties `http.Server` and `net.Server` record, the `zlib` byte counters, `StringDecoder`'s held state (`lastNeed`/`lastTotal`/`lastChar`), and `MessagePort.hasRef`/`onmessageerror`. Real values throughout rather than defaults-shaped stubs — the `process.cpuUsage` zeros were a reminder of what a plausible constant costs. Testing against real node then corrected three assumptions no documentation would have: `keepAliveInitialDelay` is kept in whole SECONDS, so 250 ms reads back as **0**; `bytesWritten` counts bytes the coder has PROCESSED, not bytes handed to `write()`, so it is 0 immediately after a write; and a fresh `MessagePort` reports `hasRef()` **false**, not true. Two probe bugs of my own along the way, both worth naming because they looked like engine faults: a `finish` listener attached AFTER `end()` never fires here, because this engine finishes sooner than node's; and `lastChar`/`lastNeed` were first attached to `TextDecoder` instead of the `string_decoder` module's class, which shares the same private field name. And one real self-inflicted bug, caught only because the fixture suite hung: adding node's `net.Server.keepAlive` — a reported option defaulting to **false** — collided with an internal tri-state of the same name where `undefined` meant *enabled*, so every server silently stopped keeping connections alive and the HTTP fixtures waited forever. The internal flag is renamed; the rule (grep for whether the engine READS a name before adding node's version of it) is in AGENTS.md |
| **Phase G — the last public clusters, and the collision rule applied backwards** | First the audit the previous boundary's rule demands: every property added across this session's boundaries was checked against whether this engine READS that name elsewhere. `readable`, `writable` and `fd` looked alarming — 15, 21 and 19 read sites — but the only generic reader is `Socket.readyState`, which never sees a stdio object. A clean result, and now a checked fact rather than a hope. Then the last public clusters the shapes sweep names: the **fetch `Request` surface** (`mode`, `credentials`, `cache`, `referrer`, `referrerPolicy`, `integrity`, `keepalive`, `destination`, `duplex` — browser concepts node still reports with fixed defaults, and an absent field reads as `undefined` and fails a comparison node would pass) and **`net.Socket`'s reported state**: `bufferSize` from the writable side, `localFamily` derived when the address arrives, `resetAndDestroy`, and `server` — the listener that accepted the socket, which node exposes and this engine had only under an internal name. The shapes total falls 66 → 51, and what remains is `Buffer`'s internal fast paths and the `http.Agent` extension points |
| **Phase G — a custom `http.Agent` was silently ignored** | The previous boundary named the Agent extension points as work that only counts if the client ROUTES through them, so this does that — and found something worse on the way in. The client selected its agent with `options.agent && options.agent._release ? options.agent : globalAgent`, testing for an internal of this engine's own Agent. **Every subclass — which is what every proxy agent is — failed that test and was replaced by the global agent, silently.** A caller passing a proxy agent got a direct connection and no indication anything had been ignored, which is a security-relevant failure rather than a missing feature. Any object offering an agent's interface is now an agent, and an agent that OVERRIDES `createConnection` decides where the request goes. Gated the way the real thing works: two servers, an agent that hands back a socket to the second, and a request addressed to the first — the body has to come from the second. Plus `defaultPort`, `protocol`, `maxTotalSockets`, a live `totalSocketCount`, and the `keepSocketAlive` / `reuseSocket` / `removeSocket` / `createSocket` hooks, each CALLED rather than merely present. The routing is deliberately narrowed to agents that override the method: the base implementation is equivalent to the existing path, and sending every ordinary request through it changed the default connect sequence and hung the http harness |
| **Phase G — sweeping the silent-substitution shape** | The agent bug had a SHAPE worth hunting rather than an instance worth fixing: *test a user-supplied object for an engine-INTERNAL field, and quietly swap in a default when it fails*. Grepping for the mechanism found no other case — the remaining `instanceof` checks are on `Request`, where node branches the same way, and the rest are internal state on our own objects. A clean sweep is still worth the grep: it turns "probably fine" into a checked fact for the price of one command. The adjacent question did find something: `https.globalAgent` reported `http:` and port **80**, because one `makeHttpModule` built its agent with default options regardless of scheme. node keeps a separate agent per protocol and callers read `defaultPort` to decide where to connect, so this is the kind of wrong value that produces a request to the wrong port rather than an error |
| **Phase G — axios runs, and the tag that silently changed its transport** | A library-level proof of the agent work, and it began by invalidating itself. axios matched 8 of 9 behaviours immediately — on the **fetch adapter**, because `Object.prototype.toString.call(process)` returned `[object Object]` here and `[object process]` in node. axios decides whether the Node http adapter is usable with exactly that check, so it silently took a different transport: every agent, proxy or pool a caller configured was bypassed without a word. The near-miss is worth as much as the bug — a green-looking 8/9 was measuring the wrong stack. `Symbol.toStringTag` is now set on `process`, `URL`, `URLSearchParams`, `Headers`, `AbortSignal`, `AbortController`, `Blob`, `File`, `FormData`, `Response`, `Request`, `Event` and `performance`, matching node. On the REAL adapter two more defects surfaced, both in the redirect path that `follow-redirects` drives: the client response had no `.req` back-reference, and **`url.format` dropped the port** whenever the object carried `hostname` rather than `host` — so a 302 from a server on any port sent the next hop to port 80. Also, the implicit `Host` header was written to the wire without being recorded, so `getHeader` read `undefined`. All nine axios behaviours now match, including redirects, timeouts, error rejection and instances |
| **Phase G — `url.parse` was one regex** | The axios proof exposed `url.format`; its sibling deserved the same look, because the same consumers use both — `follow-redirects` parses the current URL and formats the next one. `url.parse` turned out to be a single regex assuming every input was `scheme://host/path`, and it was wrong in five ways at once: **a bare path came back as `/`**, losing it completely; `user:pass@host` put the user in `hostname` and the rest in `pathname`; `mailto:` returned all nulls; and `host`, `hash` and `path` were absent entirely, so any caller reading them got `undefined`. Rewritten as a real parser — protocol, slashes, auth, IPv6 brackets, host/hostname/port, pathname, search, hash, path and query — with 13 vectors against node. The `//h/p` case is the one worth naming: without a protocol node does NOT read a host from it, because `slashesDenoteHost` defaults off, so it stays a path |
| **Phase G — finishing the `url` module** | `resolve` and `urlToHttpOptions` are the last two legacy functions on the redirect path, and both were short. `url.resolve` worked only when the base carried a scheme — it delegates to the `URL` constructor, which throws on a bare path — so `resolve('/base/path', 'sub')` returned `'sub'` instead of `'/base/sub'`, silently discarding the base. And `urlToHttpOptions`, which is how a WHATWG URL becomes `http.request` options, omitted `hash`, `search`, `pathname` and `auth` entirely and reported `port` as a STRING where node gives a number and omits the key when there is none — a request built from it would drop its credentials. 12 vectors against node. With `format` and `parse` from the previous two boundaries, every function the redirect path touches is now measured rather than assumed |
| **Phase G — `querystring`, and a repeated key that lost data** | `url`'s sibling: same legacy era, same consumers, same risk of being present, callable and quietly wrong. Six gaps, one of them data loss. **A repeated key overwrote instead of becoming an array**, so `a=1&a=2` parsed to `{a:'2'}` and every value but the last vanished — in exactly the shape most likely to be a filter list or a multi-select. `+` was not decoded as a space, so `+a+=+b+` kept its plusses. `stringify` joined an array with COMMAS into a single value no parser would split apart, wrote `null` as the text `"null"`, and ignored the custom separator and equals arguments entirely. A malformed escape now survives as-is rather than throwing and taking the whole query with it. 25 vectors against node |
| **Phase T — ink's borders, gated at last** | The standing work names ink-style TUIs, and ink WAS already exercised by the TTY harness — but only with plain ASCII, so neither phase-T fix (column-accurate widths, the alternate-screen restore) was covered by the library that motivated them. Now it is: ink renders a bordered `Box` around `abcdef`, `日本語`, `a日b` and `🎉ok`, and the frame is compared line for line against real node rendering the same app. This is a genuine agreement test — ink measures each row with `string-width` and decides where the right border goes, while the grid places the characters from its own UAX #11 table, and neither consults the other. All six rows measure 20 columns. Proved able to fail by forcing widths back to 1, which reports the frame ragged at 17/19/20. One honest limitation recorded rather than papered over: ink produces NOTHING under a non-TTY run here where node prints the frame, so the gate drives the TTY path — which is how a TUI runs on a phone anyway |
| **Phase G — `exit` and `beforeExit` were never emitted** | Chasing the lead the previous boundary left — *ink prints nothing under a non-TTY run* — and the first thing it did was correct that claim: ink prints fine if anything keeps the loop alive, so the report was wrong, not just incomplete. What is real is far bigger. **`process.on('exit')` and `process.on('beforeExit')` never fired at all.** That is where cleanup lives — flushing a log, writing coverage (`nyc`/`c8` write on exit), removing a lockfile or a temp directory — so every program doing shutdown work had been silently skipping it, and silently is the operative word: nothing errors, the work just never happens. Now emitted with node's semantics, which are fussier than they look: `beforeExit` fires only when the loop empties WITHOUT an explicit exit, re-fires if a handler schedules more work, and never fires after `process.exit()`; `exit` fires last and synchronously for every kind of exit, carrying the final code, and what it writes still reaches the output. 9 cases against node. Along the way the event loop itself was cleared: immediates, ticks, microtasks and chained combinations all drain before exit exactly as node's do |
| **Phase G — sweeping the rest of the process events** | If two lifecycle events were never emitted, the class was worth sweeping rather than the instance worth fixing. Three more: **`warning`** — `process.emitWarning` printed to stderr and never fired the event, so anything attached to observe or route warnings saw none; **`uncaughtExceptionMonitor`** — how an error reporter watches every uncaught exception without changing whether the process dies; and `multipleResolves`, left alone because it needs a hook into promise settlement that JSC does not expose, and node has deprecated it. Two corrections came from measuring rather than reading the docs: node emits `warning` **and still prints** — a listener observes, it does not suppress, and my first version made every warning invisible to anyone who attached a reader — and a monitor that THROWS is not contained, because swallowing it would hide a broken error reporter behind the error it was reporting. 6 cases plus two stderr contracts a stdout comparison cannot see |
| **Phase G — the error-path events, and a 400 nobody was sending** | The event sweep, one layer out: sockets, servers and the HTTP client. The happy path was already perfect — 13 events across four emitters, identical to node — which is a clean result worth having measured. The ERROR path is where the gap was, and that is the expected place: those events fire only when something goes wrong, so they are the least likely to have been built and the most costly to be missing. `EADDRINUSE`, `ECONNREFUSED` and socket `timeout` all already worked. **`clientError` did not** — a malformed request line was ACCEPTED and turned into a message with a nonsense method, so a server neither rejected the request nor told the peer anything, and the connection sat there. Now the request line is validated, `clientError` carries node's `HPE_INVALID_METHOD` and the socket, and with no listener the default is node's: **400 Bad Request and close**. Refusing garbage loudly is the point — accepting it silently is worse than either |
| **Phase G — error paths across the subsystems, and a verification that was measuring nothing** | The error-path sweep continued into `child_process`, `zlib`, `worker_threads` and `fs.watch`. Corrupt-input and missing-path errors already matched. **Spawning a binary that does not exist reported `exit 127` where node emits an `'error'` with `ENOENT`** — so "not installed" was indistinguishable from "ran and failed", and code that listens for a missing command never heard anything. Fixed, with node's sequence: `error(ENOENT)` then `close(-2)`, and no `'spawn'`, because the exec never happened. **The verification is the lesson here.** The first version also treated "no shell attached" as ENOENT, which made the gate pass — and it was measuring the HOST, not the command: with no shell every command returns 127, so the probe never tested a missing binary at all. The same shape as axios silently choosing its fetch adapter. Narrowed to a genuine not-found, and the gate now attaches a shell so the condition it claims to test is the one it tests. The `childevents` gate caught the overreach. A worker that throws now reaches the parent as node's `'error'` carrying the exception, then `exit 1`. The first attempt was reverted for hanging, and the retry found why: the hang came from deferring the exit so the IPC send could "flush", when the open channel is itself what holds the loop — sending and exiting AT ONCE works. The second failure was better hidden. The reporting handler lived in the `worker_threads` factory, so it was installed only if the worker imported that module, and a worker that throws on its first line never does. Moved to bootstrap. Both halves also had to land TOGETHER to work at all, and an aborted edit script meant the first test ran with only one of them |
| **Phase G — `moveMessagePortToContext`, a refusal that EXPIRED** | Fifth recorded gap to fall on re-examination, and the first whose reason was true when written. It read "contexts here are separate engines with no shared memory" — correct at the time, and obsolete the moment vm contexts became a second `JSContext` in ONE virtual machine a few boundaries ago. Nothing about this was impossible; the blocker was removed by unrelated work and nobody went back to check. **A refusal is a claim with a shelf life, and it should be re-read whenever the ground under it moves.** The port is now given a face inside the target context: `postMessage` forwards to the real port, incoming messages arrive at `onmessage`. Two node details that a plausible guess gets wrong, both measured: the returned port is WEB-shaped (no EventEmitter surface — `on` is undefined), and it stays closed until `start()` is called, queueing rather than dropping what arrives first. Also fixed a real ordering bug in the vm layer found on the way: a key added to a sandbox BETWEEN runs was bound one run late, because binding happened only after evaluating |
| **Phase G — `resolveTlsa`: a constraint on the wrong half of the job** | Applying the shelf-life rule to the rest of the list. This refusal read "TLSA carries a certificate-association hash for DANE, which needs a TLS stack that can consume it" — every word true, and about **consuming** the record rather than resolving it. On the wire TLSA is three bytes and a blob, no harder than CAA, which this resolver has parsed all along. The same shape as `crypto.diffieHellman`, refused for a bignum that applied to a different function: a real constraint fastened to the wrong half of the work. Now parsed in `NodeDNS`, returned as node's `{certUsage, selector, match, data}` with the association data as an `ArrayBuffer`, and routed from `dns.resolve(name, 'TLSA')` too. Verified against a live record — usage 3, selector 1, match 1, 32 bytes, identical to real node. Sixth gap to fall this session, and the pattern across them is one sentence: **check WHICH half of the job the reason is about** |
| **Phase G — X448, written rather than borrowed** | Seventh gap to fall, found by running the previous boundary's own question over the list: *which half of the job is the reason about?* The refusal read "X448 has no CryptoKit support" — true, and a claim about the SYSTEM, silent on whether the curve can be written. RFC 7748 is a Montgomery ladder over p = 2^448 − 2^224 − 1, and JavaScriptCore's `BigInt` does that arithmetic — the same shape as `scrypt` (PBKDF2 around a Salsa20/8 mix) and finite-field DH, both of which fell for the same reason. `generateKeyPairSync('x448')` now produces real DER (SPKI `3042300506032b656f033900`, PKCS8 `3046020100300506032b656f043a0438`) and `crypto.diffieHellman` routes X448 PEMs to the ladder. Verified the way key agreement must be: **cross-engine**, real node generating one half and this engine the other, both deriving the SAME 56-byte secret — a secret two independent implementations agree on cannot be a private convention. The arithmetic was checked against node's own values before a line of it was wired in |
| **Phase G — vite's dev server runs, and three defects it took to get there** | The real-package proof again, on the dev server the target population actually uses — and again it found a NEIGHBOURHOOD rather than a bug. (1) **Conditional exports were resolved by a fixed preference that tried "require" first for everyone.** Node keys the condition to the SYNTAX of the request: `import` sees "import", `require` sees "require", and `import()` sees "import" even from a CommonJS file. A dual package ships genuinely different code behind those, so vite loaded its deprecated CJS shim and eslint could not reach `@humanfs/node`, which publishes an "import" condition and nothing else. Subpath exports did not exist at all — `rollup/parseAst` is not a path, it is a key in rollup's map, and vite asks for it on its first line — so exact keys, `*` patterns and array fallbacks are implemented, and "exports" is now the package's ONLY surface, with node's `ERR_PACKAGE_PATH_NOT_EXPORTED` where node refuses. (2) **The ESM→CJS transpiler was rewriting text inside STRINGS.** A bundle carries JavaScript as data — vite ships the browser's `import.meta.hot` and its worker shim's `export default function WorkerWrapper` as template literals it serves to the client — and a blind replace edited code that was never ours while leaving the real `import.meta` forms nobody enumerated (`.filename`, `.dirname`, `.require`, bare) in place, where JSC refuses to parse the script at all. Every rewrite now runs through a scanner that tracks strings, template literals with `${}` nesting, both comment forms and regex literals; the statement patterns match against a MASK of that scan and are applied to the original at the same offsets. (3) Two latent transpiler holes surfaced the moment the import condition started reaching real ESM builds: `export async function*` (the Anthropic SDK's SSE reader) kept its `export` keyword because the pattern had no place for the `*`, and a leading `\s*` reaching back across the newline glued `module.exports.default =` onto the previous line's `}`. Both had been invisible while every dual package resolved to its CommonJS half. Vite's dev server now serves modules byte-identical to real node's, and `Cannot parse '<file>'` finally carries JSC's own message and line — the diagnostic that made all of this findable |
| **Phase G — the ESM grammar, enumerated instead of trusted** | The two transpiler holes the vite boundary hit were found by ACCIDENT, which is the part worth fixing: a transpiler that works by pattern is only as good as its enumeration of the grammar. So the grammar is now a corpus — twelve module shapes, each a whole little package, run on both engines with stdout compared. It found two more, both real ES semantics rather than syntax. **An ES module exports BINDINGS, not values**: assigning `module.exports.x = x` at end-of-module froze every export at the value it had then, so `export let count` read zero forever after `bump()`. And **a cycle through a default export saw `undefined`** where node sees a hoisted function, because the alias was also written at the end. Both fall to the same change: every export is now a getter defined BEFORE the body, which gives liveness, gives the hoisted function to whatever reaches back into the module mid-cycle, and throws TDZ for a `const` read too early — node's behaviour in all three. `__esModule` became non-enumerable at the same time, since node's namespace has no such key and it was showing up in `Object.keys(ns)`. What remains is pinned rather than hidden: a DESTRUCTURED named import (`import { count }`) is still a snapshot, because a CommonJS transpile binds it as a local and making it live means rewriting every reference — real scope analysis, or it captures a shadowing parameter of the same name. `import * as ns` is live and is the form that behaves; the gate pins both sides so the divergence can neither widen nor be quietly closed |
| **Phase F/G — the package manager knows which binaries have WebAssembly builds** | The vite build needed two escape hatches, and both were the same shape: rollup's parser and esbuild ship as per-platform NATIVE binaries, and iOS is a platform they will never ship for — it can neither `dlopen` a `.node` addon nor exec a downloaded executable. Both projects publish a WebAssembly build for exactly this case (`@rollup/wasm-node`, `esbuild-wasm`), versioned in lockstep with the native one. So the substitution is now the package manager's, not the harness's: `resolveTree` resolves those names against their wasm build and installs it under the ORIGINAL name, so the package depending on it is unmodified and never learns the difference, and the install line says which stood in for what. `npm install vite` on the phone now yields a vite that runs AND builds — no config, no aliasing. The gate asserts the substitution happened by reading the installed `package.json`, because the whole build rests on it. This is the same principle §4 already applies to runtimes: what a platform forbids is answered with a portable artifact, not with a lie about what was installed |
| **Phase G — the other half of the same wall: a `.node` addon says what it is** | Having given the two native packages that HAVE a WebAssembly build their substitute, the question is what the ones that do not should say. `process.dlopen` already refused with a true reason, but `require('./build/thing.node')` reported **MODULE_NOT_FOUND** — a lie in shape rather than in wording: the file is right there, and it is the platform that cannot load it. That error sends people deleting node_modules over something no reinstall can fix. A `.node` file now resolves (node resolves it too — `require.resolve` returns the path) and fails at LOAD with `ERR_DLOPEN_FAILED` and the same sentence `dlopen` gives, naming the file. Verified against real node on the same tree, which fails on a hand-written Mach-O stub too: all six outcomes carry the same code, including the ordinary missing module still being MODULE_NOT_FOUND — the new case has to not swallow the old one. **A refusal is only honest if it is about the right thing**; this one had a true reason attached to the wrong failure |
| **Phase G — breadth chosen by measurement, and a defect shape swept** | Rather than guessing the next gap, fifteen of the packages an iOS IDE's user would reach for first were installed and loaded: undici, hono, fastify, postcss, sass, dayjs, markdown-it, pino, svelte, graphql, tsx, vitest, drizzle, socket.io, nodemon. Thirteen worked untouched. **fastify did not, and the cause was a shape already seen twice**: `const { debuglog } = require('node:util')` is avvio's first line — fastify boots through avvio — and our `debuglog` was written as `this.debug(section)`, which is undefined the moment the function travels alone. `process.hrtime` had failed the same way for eslint, and `crypto.generateKey` was carrying the same defect unnoticed. So the sweep is a gate now: it destructures the twenty-nine core functions people actually destructure and calls them unbound, and it reads EVERY core module's own functions, flagging any that reaches through `this` before it nests a function of its own — the exact position where an unbound call breaks, and not the `this` inside a returned function, which is forwarding the caller's and is what `promisify` and `callbackify` correctly do. The remaining breadth failure is recorded rather than fixed: `tsx` wants `module.register()`, node 20.6's ESM loader hooks, which this engine claims a version for and does not have |
| **Phase G — `node app.ts`, and a program that failed by printing nothing** | The breadth sweep's one refusal was tsx, which wants node 20.6's `module.register()` loader hooks. The hooks are the mechanism; the thing people want is to run TypeScript, and node itself grew that in 22.6 (`--experimental-transform-types`). So the engine compiles `.ts`/`.tsx`/`.mts`/`.cts` with the project's OWN typescript package — the ts-node model, and the only honest one available, because type erasure is not a text substitution: an enum is an object at runtime, a parameter property is a declaration with no JavaScript equivalent, and a partial stripper would be WRONG on real code rather than merely incomplete. Modules are left as ESM so the ordinary ESM→CJS path handles them, which keeps live bindings and everything else identical to a `.js` file's. Verified against **real node's own transform-types** — two independent TypeScript implementations, node's and Microsoft's, agreeing on what the program prints across generics, enums, `satisfies`, abstract classes, type-only imports and parameter properties. A project with no typescript installed is told exactly that, rather than shown a SyntaxError pointing at a type annotation. **And the probe that found this exposed something worse**: a syntax error in the ENTRY script printed nothing at all and exited 0 — a program that does not run and does not say so, which is the worst outcome available, since every other failure at least announces itself. It now prints the file and JSC's SyntaxError and exits 1, as node does |
| **Phase G — the project's own tsconfig, and `@app/*` is not a package** | Running TypeScript with hardcoded compiler options is wrong the moment a project sets `target`, `jsx` or decorators — so the nearest `tsconfig.json` is now read by the project's own typescript (`readConfigFile` + `parseJsonConfigFileContent`), which is the only thing that parses JSON-with-comments and resolves `extends` correctly. `module` stays forced to ESNext so the ordinary ESM→CJS path still owns the semantics. **`paths` aliases resolve too**: `@app/greet.ts` is neither a package nor a path, it is a convention the project declared, and TS projects are full of them — tried before the node_modules walk, as every TS runner does. Resolving one requires typescript, and requiring typescript is itself a resolution, so a re-entrancy guard stands between the two; without it the first bare specifier in a TS project recursed until the stack gave out. Verified against **tsx on real node reading the same tsconfig** — target, decorators and aliases all agreeing. Found on the way: `./tool.mjs` and `./app.ts` typed at the prompt were handed to the SHELL, which tried to run JavaScript as `sh`; only `.js` had ever been routed to the engine |
| **Phase G — chasing vitest into four defects, and stopping where the evidence stopped** | vitest composes everything just built — TypeScript, vite, workers — so it was the natural next proof, and it found four real defects on the way in. (1) **`SharedArrayBuffer` did not exist**, and no JSC option turns it on — but `Atomics` DO work on an ordinary ArrayBuffer, measured, and the constructor's absence is a different thing from the sharing it names: tinypool allocates one per worker for its own signalling, so a pool could not start at all. It exists now, backed by a real buffer, and the SHARING is where the refusal lives — crossing to another engine throws rather than arriving as an unrelated copy nobody would debug. (2) **Child IPC was stricter than node's.** The earlier nine-site sweep onto the structured clone codec took this site too, but node's `child_process` channel defaults to `serialization: 'json'` and only 'advanced' clones — so a message carrying a function threw here where node drops the property, which is exactly what tinypool sends to every worker. Both modes are honoured now, and `worker_threads` is exempt because it has no json mode at all — a Map posted back from a worker stays a Map, which the clone-consumers gate caught the moment it did not. (3) **`MessagePort` carried its internals as own enumerable properties**, where node's has none; real code spreads a port (`{ ...message, source: 'port' }`), and the peer pointer made that spread cyclic. (4) **Source compiled at RUNTIME could not import.** `new Function('s', 'return import(s)')` is the standard way to keep a dynamic import out of a CommonJS transpile — the loader's rewrite never sees it, and JSC's own import has no module loader: "No module loader provided." The Function constructor now applies the same code-only rewrite. All four are gated against real node. **vitest itself still stops at the worker handshake** — by hand the same child answers `ready`, in the real run it does not and prints nothing on either stream. That is recorded as a diagnostic rather than as prose, because a lead in prose rots and a lead in the suite does not |
| **Phase G — thirty packages, and a legal ESM cycle that this engine made illegal** | The breadth sweep doubled to twenty-nine packages, which is how the next three defects were found rather than guessed. (1) **A destructured import turned a legal cycle into a ReferenceError.** execa's `send.js` and `strict.js` import each other; real ESM reads a binding where it is USED, by which time the cycle has closed, but `const { sendMessage } = require(…)` reads it at import time, mid-evaluation, in TDZ. Imported names now come through `__esmBinding`, which reads the value when it can and otherwise returns a forwarder that reads it at the call — a path reached ONLY where the alternative is today's throw. (2) **`export const { a, b: c } = …` had no pass at all**; signal-exit writes one spread over a dozen commented lines. It needed a balanced scan of the pattern and a distinction a regex cannot make — in `{ b: c }` the binding is `c` and `b` is a key. (3) `SharedArrayBuffer.prototype` was a bare object, so `getOwnPropertyDescriptor(SharedArrayBuffer.prototype, 'byteLength').get` — how jsdom and mongoose tell buffer kinds apart — read `undefined`; it is ArrayBuffer's prototype now, because the object IS one. **27 of 29 packages load**, and the two that do not are honest: tsx wants `module.register()`, and sharp is a native addon refusing in its own words. Both new grammar forms are pinned in the corpus, which is now thirteen shapes |
| **Phase G — three ways to start a child badly, all of them silent** | Still chasing vitest's worker, and the chase kept finding defects even where it did not find the cause. (1) **A typed env value dropped the whole environment.** node coerces env values to strings and callers rely on it — vitest passes `PROD: false` — but the child's env was cast to `[String: String]` in one go, so one boolean made the cast fail and the child silently received NOTHING it had been given. Values are coerced per key now, with booleans told apart from the `NSNumber` they parse into, since "true" and "1" are different strings to the program reading them. (2) **An unreadable child script became the program.** The path was used unnormalised, so `…/dist/index.js/../entry/process.js` — which is how tinypool names its worker — was handed to the OS, which refuses it because index.js is a file; and the fallback then ran the PATH STRING as the source. A child that starts, does nothing, says nothing and never exits is indistinguishable from a child that is waiting. It normalises now, and a script it cannot read is `Cannot find module`, out loud, exit 1. (3) **A forked child never exited.** The IPC channel held the loop open merely by existing, where node's hold belongs to a message LISTENER — which is the thing that says more work may arrive. A worker that sends its result and listens for nothing now exits, as node's does. All three are gated against real node. **vitest still stops at the same place**, and the diagnostic now records what has been RULED OUT by isolated replication — message shape, back-to-back messages, fork options, a replaced environment, a parent in top-level await, eight concurrent children, dot segments — which is what makes the next attempt cheaper than this one |
| **Phase T/G — a dev server at the prompt, and what a terminal does when a program clears** | Every node gate so far ran a program to COMPLETION; nothing had exercised the path with a program that does not exit, which is what a dev server is. Through msh, as a user types it: the shell hands the program to the phase-T launcher and the prompt comes straight back, the output streams to the scrollback while it runs, the server answers requests from outside the app, and ^C ends it with the port released — nothing here tears a process down for us. That path was sound. What was NOT: **vite's banner arrived under two dozen blank rows**. Vite clears the screen before printing, the way readline does — home the cursor, `ESC[0J` to erase below, after printing `rows − 2` newlines to push the old content up — and a transcript that strips escapes turned all of it into empty lines. Two fixes, both about what a terminal actually does: a line that was ENTIRELY escapes is not a blank line, since a cursor move is not something a terminal prints; and erase-below with nothing below it is a CLEAR, so it clears the scrollback. `npx vite` from the prompt now reads exactly as it does on a laptop — version, blank, the URL — and it serves a transformed TypeScript module to a client that is not us |
| **Phase G — hot reload, the half that makes a dev server worth running** | Four subsystems that had only ever been tested apart, in one chain: kqueue `fs.watch` under chokidar, vite's module graph, an HTTP upgrade on our own socket server, and a WebSocket carrying the message. The peer is the SYSTEM's WebSocket client rather than ours — the browser's side of the wire, standing in for the browser. It works untouched: the handshake answers `{"type":"connected"}`, an edit written to disk the way an editor writes it reaches the watcher, and vite pushes a real `js-update` naming the module that ACCEPTED it (main.ts, on label.ts's behalf — which is what `import.meta.hot.accept()` means, and the assertion had to be corrected to say so). **No engine change was needed, and the gate is the whole deliverable**: a capability nothing exercises is a capability that rots, and this one crosses four subsystems any of which could break it silently |
| **Phase F — `npm run`, the first line of every README** | msh's npm had `install` and `ls` and nothing else, so the way every project is actually started did not work. `npm run <script>` now runs the package.json script as an ordinary msh program — which is what makes a script's `&&`, its pipes and its env prefixes behave, and what lets a long-running one take the terminal exactly as it does when typed by hand. Bare `npm run` lists; `npm test`/`start`/`stop`/`restart` are their own scripts; `pre` and `post` hooks run around the main one; arguments after `--` are appended; the status propagates; a missing script says `Missing script` and exits 1. The environment npm gives a script (`npm_lifecycle_event`, `npm_lifecycle_script`, `npm_package_name`/`_version`) is set on the SHELL for the duration rather than as a command prefix, because a prefix reaches only the first command and scripts are routinely `a && b`. Verified against **real npm** on the same package.json — eight invocations matching on output AND exit status. Two defects fell out of it: the package.json is the one governing the CURRENT directory, found by walking up, not the workspace root's (a project in `app/` is the ordinary layout); and **`node -e` was putting a phantom script path in `process.argv`**, so every `node -e "…" value` script read the wrong argument — node's eval form has no path, argv[1] is the first real argument. The dev-server gate now starts vite through `npm run dev` |
| **Phase F — a project's first five minutes, end to end** | `npm create vite@latest app` is how a project BEGINS, and msh had no `create` at all. It does now — npm's rule is that `create <name>` runs the package `create-<name>`, scopes and existing `create-` prefixes included, with `init <name>` as its older spelling and bare `npm init` writing a package.json. The whole first run is one gate: **scaffold, install, `npm run dev`, then a request from outside the app for a module vite compiled** — every step a different subsystem, and they had only ever been tested apart. Two defects fell out. `npm install` read the workspace root's package.json rather than the one governing the CURRENT directory, so `cd app && npm install` after scaffolding found nothing (the same defect `npm run` had, at the site nobody re-checked); and the `--` separator was passed through to the scaffolder, which read it as the project name. **And a new gap, measured rather than guessed**: vite 7 bundles with rolldown, whose binding is native. The package manager now installs the `-wasm32-wasi` optional binding a napi-rs package publishes — the same principle as the rollup and esbuild substitutes, in the shape napi-rs uses — and that binding loads all the way to one missing piece: **`node:wasi`**. The gate PINS that, so the day WASI lands the pin breaks and says so. It is the largest remaining lever in phase G: rolldown, oxc, lightningcss and every other napi-rs package ships a wasi build |
| **Phase G — `node:wasi`, written out, and the wall that was actually behind it** | The previous boundary pinned vite 7's bundler as blocked on `node:wasi`, so that is what got written: **WASI preview1**, forty-odd syscalls over this engine's own fs — args and environ, both clocks, `random_get`, the fd family (read, write, seek, tell, fdstat, filestat, readdir, close), the path family (open, filestat, create/remove directory, unlink, rename), preopens, and `proc_exit` as a throw that `start()` unwinds. Verified against **node's own WASI on a wasm module assembled BY HAND here** — 143 bytes of real WebAssembly in the house tradition (tar, gzip, ICMP, DER): it imports `fd_write`, owns a page of memory, and writes a string on `_start`. Both engines run those bytes, and then twenty more syscalls are driven directly through the import object, all identical. The one thing that first version got wrong is the thing WASI is FOR: **rights**. A descriptor carries the capabilities it was opened with, and node refuses an operation outside them with `ENOTCAPABLE` — ignoring them makes a sandbox that is not one, and the gate caught it as three lines of difference. **Then the pin moved, which is what a pin is for**: rolldown now gets past `node:wasi` and stops one wall further back — its wasm is built with THREADS, and a module declaring shared memory does not parse in JavaScriptCore. That is a platform fact rather than something to write, and it has its own gate now, because the constructor `new WebAssembly.Memory({shared: true})` is ACCEPTED and quietly returns ordinary memory: anything feature-detecting on the constructor would miss it. The claim it corrects was mine, one boundary old |
| **Phase G — how far a Rust-native package gets, measured on two of them** | With WASI written, the question is what it reaches. A napi-rs package ships a `wasm32-wasi` build for platforms it has no binary for, and its generated loader tries this platform's `.node` first and only falls back when told — through `NAPI_RS_FORCE_WASI`, a flag its own authors provide. On iOS the native branch can never load, so that flag is not a preference here but a statement of fact, and the engine sets it by default; without it oxc's loader never even ATTEMPTED the portable build, and the error it printed named a missing darwin binary — true, and useless. With it, both **oxc-parser** and **rolldown** install their WebAssembly build and reach it, and both stop at the same place: the build is compiled with THREADS, and a wasm module declaring shared memory does not parse in JavaScriptCore. Two packages sharing nothing but their build system, one wall, and it is the platform's rather than ours. **That settles the shape of §4's runtime plan**: a single-threaded wasi build — which is what CPython's is — is on the far side of this line, and every napi-rs build is behind it until JavaScriptCore enables shared memory |
| **Phase G — React on a phone, which is esbuild doing a dev server's real work** | The vite gates so far served plain JavaScript and TypeScript, which vite handles itself. A `.tsx` is different: vite hands it to **esbuild**, and esbuild here is the WebAssembly build the package manager substitutes, driven through a live child process — so a React project is the first thing that makes the wasm compiler do a dev server's continuous work rather than a one-shot transform. A real project shape was used, `@vitejs/plugin-react` and all, and every layer showed up in what was served: types erased, JSX compiled to React's AUTOMATIC runtime (`jsxDEV` from `react/jsx-dev-runtime`, which is the plugin's doing and not esbuild's default classic transform), Fast Refresh registrations woven in by babel over esbuild's output, the HMR client wired to the module, and `react` itself PRE-BUNDLED into `.vite/deps` — dependency optimisation, which is esbuild bundling a package on the device. No engine defect this time; the gate is the deliverable, and the first assertion it caught was mine (without the plugin, vite's default JSX transform is the classic runtime, so checking for `jsx` was checking for the wrong thing) |
| **Phase G — the line number in a stack trace, which an editor turns into a click** | Everything this engine does to a file before running it moves lines, and nothing had ever checked. **A CommonJS module's trace was off by one** — the wrapper took a line of its own — and an **ES module's by four**, because the prologue of live-export getters (added this session) sat above the body; a multi-line import collapsed to one line moved everything below it up by two more. A trace that points four lines past the throw is worse than no trace: it sends you to code that is fine. Three fixes, all the same idea — the body's first line has to BE the file's first line: the wrapper no longer ends in a newline (node's does not either), everything the ESM transform adds rides on that same line or the last one, and a replacement now carries the newlines it consumed so later lines keep their numbers. **And the thing node prints that we did not**: the file, the line, the line's own TEXT and a caret, above the stack — a trace tells you where, that tells you what is there. Gated on five shapes against real node, including a throw inside a callback and a caught error's own `.stack`. Columns are deliberately not compared: JavaScriptCore and V8 disagree about where in an expression an error happens, which is the engine's property and not this transform's |
| **Phase G — and the same question one layer up: TypeScript** | The wrapper and the ESM prologue were fixed by writing them not to move lines. A TypeScript file cannot be fixed that way, because erasing types is not a rewrite that preserves anything: four lines of `interface` above a function simply STOP EXISTING, and an `enum` expands into more lines than it occupied — so traces were off by seven in one direction and by two in the other, and the new source-context header quoted whatever happened to sit there, confidently. The compiler already emits the answer: `transpileModule` returns a source map when asked, so it is asked, decoded once per file (VLQ, whose numbers are DELTAS accumulating across the whole file — the part that catches a segment-by-segment reader), and used in the two places that render a position: the header, and every frame of the stack. This is what `source-map-support` does for node, in the one place here that renders them. The gate now covers both directions of movement, checked against node's own `--experimental-transform-types` |
| **Phase G — a trace out of a BUNDLE, and two defects found on the way to one** | An installed CLI is a bundle, and a trace into line 4000 of one tells a reader nothing. Every bundler emits a source map for exactly this; node reads one only under `--enable-source-maps`, and here it is the DEFAULT — deliberately, because this is an editor — for both spellings, a `data:` URI carried inline and a `.map` beside the file. The decoder from the TypeScript boundary needed one thing it had not: the source INDEX, since a bundle's map has many inputs and a position without its file names the bundle, which is the thing the reader was trying to get out of. Now the header quotes the author's file and every frame names it. **Two defects surfaced getting there, both from esbuild being the bundler under test.** Its `buildSync` blocks on `Atomics.wait` over shared memory — impossible here, since a worker is a separate engine — and JavaScriptCore's own message names SharedArrayBuffer, which reads as a type error and sends the reader to the wrong question; it now says what the wall is and that the async form works. And **numeric open flags were being stringified**: Go's wasm runtime opens with `O_WRONLY|O_CREAT|O_TRUNC` as a NUMBER, `String(flags)` made that "1537", which contains neither 'w' nor 'a', so the open read as read-only and every write appended instead of landing at the descriptor's position — content in the file twice. Both gated against real node |
| **Phase G — the lead from last boundary, closed, and the shape swept** | esbuild could not write into a subdirectory or emit its `.map`, and that was left unproven rather than half-recorded. It was the numeric-flag defect: without `O_CREAT`, opening a file that does not exist is ENOENT, which is exactly what it said. All four shapes work now — root, external map, an existing subdirectory and one esbuild CREATES — and the bundle gate uses esbuild's own output for both spellings instead of a map constructed here, so the lead is closed by a gate rather than by a sentence. **Then the shape was swept**, since a defect has one: a value turned into TEXT and then read as text. Widening the flags gate to node's other spellings of the same options found two more. `writeFileSync`'s `{ flag: O_WRONLY | O_APPEND }` — a number — read as "truncate", so an append silently replaced the file. And **`writeSync` has two signatures whose arguments are ordered differently** — `(fd, buffer, offset, length, position)` and `(fd, string, position, encoding)` — so a string write landed at the wrong offset, silently, which is the worst way for a write to be wrong. Sixteen behaviours now match real node, each engine in its own tree because an exclusive-create test is a trap otherwise: whoever runs first leaves the file behind |
| **Phase G — the second spelling nobody reaches for first** | The fs boundary found three defects in one shape — an option node accepts in more than one form, implemented in only one — so the shape got a sweep of its own across the surfaces a program touches most: extra arguments forwarded to a callback, options objects where positional arguments also work, and Buffer's overloads. Twenty-two call forms against real node, **four of them wrong**. `fs.readSync(fd, buffer, { offset, length, position })` — node 13's form, which packages use — read nothing and said nothing, and its async twin with it. `fs.mkdirSync(path, { recursive: true })` returned undefined where node returns the FIRST path it had to create, which is how a caller learns whether it made the tree. And **`Buffer.alloc(6, 'ab')` filled with 'aaaaaa'** instead of 'ababab': a string fill is a PATTERN that repeats, and taking its first character quietly produced the wrong bytes — the kind of wrong that surfaces as corruption somewhere else entirely. The lesson is the same one the JSON.stringify sweep taught: **an engine that implements one spelling of each call looks complete until a package uses the other**, and the only way to find those is to enumerate them |
| **Phase G — the same sweep where the forms are ASYNCHRONOUS** | Twenty-two sync call forms found four defects, so the method was pointed at the surfaces where an overload is easier to get half right because the answer arrives later: a stream's three-argument write, `Readable.from`'s several sources, `read(size)`, events' argument carrying, timers/promises' values, crypto's encodings, `promisify`, and the four shapes of `listen()`. Twenty forms, **five wrong**. Two shared a cause: **a string chunk was handed to `_write` undecoded**, so `write(text, 'hex')` meant the characters "68690a" rather than the bytes they spell, and `setDefaultEncoding` did nothing — node decodes with the given or default encoding and then tells `_write` 'buffer', which is what `decodeStrings` means. **`read(size)` ignored its size** and returned everything, so the second read saw null and read as end-of-stream — which is precisely how a framed protocol breaks, reading its header and then finding no body. `timingSafeEqual`'s length mismatch threw without node's `ERR_CRYPTO_TIMING_SAFE_EQUAL_LENGTH`, and `util.promisify` ignored a function's own `promisify.custom`, handing back a wrapper around a callback shape that was never a callback shape. Nine defects across two sweeps of the same idea: **enumerate the spellings, do not wait for a package to find them** |
| **Phase G — `path`, which every tool touches and nothing had gated** | The parts of `path` with a gate were the ones some package had already broken; the rest had never been compared. One hundred and ten results against node, on the inputs that are actually hard, and **a whole family was wrong: the trailing separator**. `normalize('/a/b/')` dropped it, and it is load-bearing — it is the difference between naming a directory and naming a file, and anything that joins onto the answer inherits the mistake. `dirname` and `basename` did the opposite, treating it as a component, so `basename('/a/b/')` was the empty string where node says 'b'. `resolve` has to STRIP it (it answers with a canonical path) while `normalize` keeps it, which is the kind of asymmetry only a comparison finds. Three more: `extname('..')` answered '.', but a name that is all dots has no extension; `basename('/a/.txt', '.txt')` stripped the name away to nothing instead of leaving '.txt' whole; and `parse('c.txt').dir` said '.' where node says '', so `format(parse(x))` rebuilt './c.txt' from 'c.txt'. And **`path.win32` was posix wearing a different separator** — a comment admitted as much — so it is now real: backslashes, drive-letter absoluteness, and its own parse and format |
| **Phase G — Buffer, where a wrong answer keeps quiet** | Fourth sweep, chosen because the failure mode here is SILENCE. **`buffer.slice()` returned a copy where node returns a VIEW** — Uint8Array's own slice copies, and inheriting it meant every write made through a slice went nowhere: a parser patching a header in place did nothing at all, and nothing reported anything. (`subarray` was already right, which is how the pair diverged unnoticed.) Seven more in the same module: `compare`'s ranged form ignored its ranges, so comparing one field of two records said the records differ; `lastIndexOf` was simply missing and answered −1; `fill` ignored start and end AND treated a string as a single byte rather than a repeating pattern; `byteLength(text, encoding)` answered the utf8 length whatever the encoding, so a caller sizing a base64 or utf16le buffer allocated the wrong thing; and `Buffer.from('abc', 'hex')` produced two bytes where node stops at the last COMPLETE one. Thirty-one results now match. Four sweeps, twenty-three defects, all in code that passed every package proof in the suite — which is the argument for enumerating a surface rather than exercising it |
| **Phase G — what the user actually READS** | Fifth sweep, over `console` and the `util.format` it runs on, because in an editor the console IS the debugging surface and a line that renders wrong is a defect the reader carries. Fifty-six lines, four wrong, and the ratio says the console work was done carefully the first time. **`%c` was printed literally and its argument appended** — it is a browser's CSS hook, which node CONSUMES and renders as nothing, so `console.log('%cbold', 'font-weight:bold')` showed the stylesheet. And **`util.inspect` accepted three options and ignored them**: `breakLength` and `compact: false` (which is how a caller asks for one entry per line) did nothing, and `maxArrayLength` was hardcoded to 100 — the cap was even consulted through a `> 100` test, so asking for less could never take effect. Arrays and objects now go through one line-breaking rule that node's own reads like. One difference was left ALONE on purpose: `%j` on a BigInt throws in both engines with different wording, because that message belongs to JavaScriptCore's `JSON.stringify` rather than to us — the gate compares that it threw, and the type |
| **Phase G — `fs/promises`, a parallel surface nothing had said anything about** | `const { readFile } = require('fs/promises')` is the first line of half of npm, and it is a PARALLEL API rather than a wrapper — so none of the sync gates said anything about it, including `FileHandle`, which is an object with its own methods and its own lifetime. Thirty results: readFile in every option spelling, appendFile, access, stat and lstat, mkdir recursive, readdir with `withFileTypes` and `recursive`, rm with `recursive` and `force`, copyFile, rename, mkdtemp, realpath, and a FileHandle read, write, readFile, stat, truncate and close — **twenty-nine already correct**, which is the answer this sweep was for. The one difference was mine, introduced two boundaries ago: `mkdir(path, { recursive: true })` returned the path RESOLVED, where node returns it in the caller's own terms ('deep', not '/deep') because the caller is about to print it or join onto it. **A fix that is right about the value and wrong about its form is still wrong**, and only a comparison catches that |
| **Phase G — HPACK, the hard half of the last big gap** | The API sweeps had dropped to one defect in thirty, which is the signal to go back to capability. HTTP/2 is the largest documented gap left, and its difficulty is not the framing — it is **HPACK**, header compression with a canonical Huffman code and a dynamic table both ends have to keep identically. Written: prefix integers, string literals in both spellings, the five field representations, the dynamic table with eviction by size, and Huffman in both directions (encoding packs bits and pads with ONES, which is EOS's prefix and how a decoder tells padding from another symbol; decoding walks a tree built once from the table). **The tables are transcribed from RFC 7541's appendices, not retyped** — 257 Huffman codes and the 61-entry static table, parsed out of the RFC itself, because a transcription can be checked against its source and a retyping cannot. Verified the same way: the RFC's OWN examples, also extracted rather than copied — three requests over one connection and three responses over another, in both spellings, which tests the dynamic table's history as much as any single encoding, since the second message refers back to what the first added. Twenty-nine checks, and the one real defect the vectors caught was a threshold: Huffman is used when it is NOT LONGER, where the first version required it to be strictly shorter — the RFC encodes a three-byte value that Huffman leaves at three bytes, and an encoder that demands an improvement disagrees with it |
| **Phase G — and the other half: HTTP/2 answers real node** | The framing on top of last boundary's HPACK: the 9-byte header, the connection preface, SETTINGS and its ACK, HEADERS with CONTINUATION (a header block split across frames, with nothing allowed between them), DATA with window credit returned immediately, PING answered, GOAWAY, RST_STREAM. `http2.createServer` is real — node's own `'stream'` API, on our own TCP sockets. **The peer is real node's http2 client**, which is the only test that counts for a protocol: it speaks nghttp2, so a frame that is wrong anywhere gets a connection error rather than a negotiation. Eight exchanges pass — headers and bodies both ways, a 404, three streams multiplexed over ONE connection, a 40 KB body across frames, and a PING — and they passed on the first run, which is what building on a codec that was already verified against the RFC buys. **The `refusal-truth` fixture caught the one mistake**, and it was in the WORDING: the new `http2.connect` refusal said 'not available yet', which is the exact phrasing that gate exists to catch — a refusal that promises rather than explains. It now says the client half is not written, and that the server half is. What stays refused is `createSecureServer`: h2 over TLS needs ALPN on a socket this platform does not expose, which is the same wall `https.createServer` names |
| **Phase G — HTTP/2's client half, and a defect under it in `net`** | `http2.connect` is real now, and the gate runs the protocol BOTH ways against real node: its client against this engine's server, and this engine's client against its server, eight exchanges each. Getting there found a defect a layer down. The first attempt sat silent — no response, no error — and node's server had answered with **GOAWAY**, which is a peer saying the connection was wrong from the start. Two causes, both about ORDER. **`socket.write()` before the connection completes was being dropped**, where node queues it and flushes on 'connect' — a client that sends the moment it asks for a socket is ordinary, and losing those bytes is silent. Fixed in `net`, where it helps everything. And the deeper one: the connection PREFACE must be the first bytes on the wire and SETTINGS the first frame, so a session that lets a request race the handshake puts HEADERS in front of both. The session owns an outbox now and orders its own frames rather than trusting listener registration order — which is the sort of thing that works by luck until a peer is strict. The `refusal-truth` fixture caught the stale claim again, for the third time this session: a refusal that named the client half had to go when the client half arrived |
| **Phase G — gRPC, and the stream defects only it could find** | HTTP/2 passed against real node, so the next question was whether a REAL library that lives on the parts of the protocol nothing else uses would run: `@grpc/grpc-js` is pure JavaScript over node's `http2`, and it needs trailers to carry the status, response headers that say 200 long before the call has succeeded, and a server-streaming call held open across many DATA frames. It runs here now, as BOTH peers, with real nghttp2 on the other side of the socket each way. Getting there cost six defects, and five of them were invisible to every fixture that came before. **`http2.connect` ignored `options.createConnection`** — a caller that dials its own socket and asks for HTTP/2 over the result got a second connection to the same address, and spoke into it while watching the first. **The client never announced the peer's SETTINGS**, which is the event grpc waits on before it will send anything at all; both sessions parse and emit it now, from one shared decoder. **The server's stream was a bare EventEmitter**, so `pause()` did nothing and the first message arrived DURING the caller's constructor — grpc pauses on line one and assigns its decoder on line twelve, and met its own half-built object. It is a real Duplex now, and Duplex rather than Readable for a reason the next defect names. **`close` was emitted when the READABLE side ended**, not when the stream was destroyed: a request handler that had only just received its request body was told the peer had hung up, and abandoned the response it was about to write. Node emits it on destroy, and a duplex is not destroyed while its writable half is open — this one was in `stream` itself, under everything. **`'response'` carried no frame flags**, and END_STREAM on the response block is exactly how gRPC answers a rejected call — the status was in the only frame it sent, and a client that cannot see END_STREAM reads it as a bodyless success. **Streams had no `rstCode`**, so a reset read as "code undefined". The `constants` table also gained what callers actually destructure: a missing entry there does not fail loudly, it writes a header literally named `undefined` |
| **Phase G — the stream lifecycle, swept** | The `'close'` defect the gRPC boundary turned up was in `stream` itself, and it was found by accident — which meant the lifecycle surface had never been compared with node at all. Everything before it asked whether the BYTES arrived. This asks the other two questions a caller actually depends on: which events fire, in what order, and what state the stream reports afterwards. Thirty-three scenarios — every readable and writable end, destroy and error path, duplex half-open, transform, pipe, unpipe, pipeline, `finished`, async iteration — run identically on real node and on this engine and are diffed event for event and flag for flag. **Thirty-one of thirty-three differed on the first run.** Twenty root causes: `'pause'` and `'resume'` were never emitted AT ALL, so nothing could pace a producer; `readableFlowing` was two-state where node's is three; a `'readable'` listener did not take the stream out of flowing mode, so the two read modes were not exclusive and a consumer reading with `read()` found the buffer already drained into somebody else's `'data'`; `autoDestroy: false` was ignored on both halves; `errored` was never set, so a caller could not find out WHY a stream died; `push()` after EOF was accepted in silence; `write()` after `end()` emitted a codeless error and still announced a clean finish; an error out of `_write` left the stream alive and writable; the end callback ran after `'finish'` instead of before; `allowHalfOpen: false` did nothing; a Transform announced `'finish'` before its own output; `'end'` fired when EOF was PUSHED rather than when the consumer had read it; `'readable'` fired once per push where node coalesces per tick; a flowing stream with an empty buffer buffered anyway, where node delivers synchronously once its sync phase is over; a finished pipe never unpiped or paused its source and a destroyed source never unpiped at all; `pipeline()` left its destination open when it failed; the async iterator consumed by FLOWING, resuming and pausing around every chunk, instead of reading; abandoning an iteration with `break` did not abort the source; and `Readable.from` was EAGER — it drained the whole iterable into the buffer before the first read, so an endless generator never returned. **Then the suite found what the sweep could not.** Making `'end'` wait for a read is right — node's own docs say it — but a read that finds the stream ALREADY OVER is still a read, and gating on delivered data hung every operator that yields nothing (`take(0)`, `drop` past the end, `Readable.from([])`). And it exposed a defect underneath: a socket whose peer hangs up promptly was DESTROYED with bytes still unread, so the tail of the transfer was lost and the `'end'` its consumer was waiting for never came — node lets what already arrived be read first. And it caught what the sweep called an improvement: node delivers a chunk SYNCHRONOUSLY when a flowing stream has nothing queued, matching it moved two orderings closer, and it broke gRPC — a caller that hands a decoded message to its own listener from inside `push()` gets re-entered halfway through its state machine and drops the message. Every chunk is buffered here, and the orderings it would have bought are pinned anyway. The last one was subtler: being read is a HISTORY, not a current state. A consumer that takes the last chunk and pauses inside its own handler has read to the end, and gRPC does exactly that — it pauses to process and will not resume until it hears the end, which is a deadlock against a stream waiting to be resumed before it announces one. One of the suite's own pins also turned out to be stale in our favour: it had been fixed to OUR older value, and real node agrees with the engine now, so it is asserted against node again. Twenty-five scenarios are byte-identical. The remaining eight are PINNED, not waived: they agree on every event and every flag and differ only in which of two already-queued callbacks runs first, which node decides by the shape of its own resume path and no documentation orders — the gate still fails if the events, the state, or those traces change |
| **Phase G — EventEmitter, the layer under that one** | The stream sweep found its worst defects in machinery every other subsystem sits on. `EventEmitter` is the layer under THAT: streams, sockets, http, workers and child processes are all built on it, and nothing here had ever checked its own semantics — only the event SEQUENCES that streams produce. Forty-one behaviours, run on both engines and diffed. **Thirteen were wrong on the first run, and three of them were whole features that did not exist.** `'newListener'` and `'removeListener'` were never emitted at all — the meta events a caller uses to attach a source the first time somebody subscribes, and to release it when the last one leaves; a library that manages resources that way was managing nothing. `errorMonitor` did not exist either, so code that OBSERVES failures without taking responsibility for them saw none. Beyond those: `listeners()` handed back the `once` WRAPPER instead of the function the caller gave (so comparing against your own listener failed); `eventNames()` missed symbol-named events entirely and kept reporting names whose last listener had gone, which reads as a live subscription; `listenerCount(name, listener)` ignored the listener and returned the total; `getMaxListeners()` said Infinity and `setMaxListeners` did nothing, so the leak warning that is the only thing the limit DOES could never fire; `emit('error', 'a string')` threw a bare Error where node throws `ERR_UNHANDLED_ERROR` with the value attached; `captureRejections` was accepted and ignored, so an async listener that threw became an unhandled rejection nobody could attribute to the event that caused it; and `events.once()` left its own `'error'` listener on the emitter afterwards — a subscription the caller never made, on an object that usually outlives the promise. Two more came out of testing what had just been written: the static `events.setMaxListeners(n, target)` was a no-op, and `process.emitWarning` delivered synchronously where node hands the warning to the loop, which is observable to anyone who attaches and removes a listener around the code that triggers one. All forty-one identical now |
| **Phase G — timers, and the tick queue's real place** | The other substrate: everything async here is scheduled through timers, and no fixture had ever compared them with node. Thirty-three behaviours, and only orderings node ACTUALLY guarantees — a bare `setTimeout(0)` racing `setImmediate` is unordered in node itself, so nothing here depends on how that race falls. Seven were wrong. **The deepest was the tick queue's place in the loop.** node runs `drain ticks; run microtasks; repeat` — the tick queue runs at the EDGES of a microtask checkpoint, never inside one. Ours rode a microtask (`Promise.resolve().then(drain)`), so a tick queued from a promise reaction cut in front of the reactions queued beside it: node reports `promise, microtask, tick`, we reported `promise, tick, microtask`. Removing the self-scheduled drain entirely broke everything else: a tick queued while no host callback is in flight has nothing to wake it. The fix needed the one thing JavaScript cannot see about itself — WHICH SIDE of the edge the caller is on. The engine tracks it, because the HOST knows: the entry script and every loop callback run on a host frame, anything else is a promise reaction. From a host frame the queue drains on the way out, before any microtask; from inside a checkpoint it rides a microtask and yields once more, so it lands behind the reactions queued beside it rather than in front of them. Getting it wrong in either direction is visible — one way a tick jumps its siblings, the other way it arrives after them — and the suite caught both. The rest were the handle and its rules: `unref()` did nothing and `hasRef()` always said true, so a watchdog held the process open forever; `refresh()` was a no-op, so the idle-timeout pattern it exists for silently never restarted; `clearImmediate` was an EMPTY FUNCTION, so a scheduled immediate could not be cancelled at all; a non-function callback was accepted and simply never ran, where node throws `ERR_INVALID_ARG_TYPE`; and every delay under 1 — negative, NaN, undefined, a value from a subtraction that went the wrong way — became 0 rather than node's 1, while one over 2^31 waited 25 days instead of clamping to 1 with a `TimeoutOverflowWarning`. **The corpus also found a bug in node.** `clearImmediate({})` on a plain object does not throw: it quietly corrupts node's immediate queue, and every `setImmediate` for the rest of that process is scheduled and never runs. It is undocumented input, so the sweep does not assert it — but it says so where it stops, because a scenario dropped without a reason reads as one that was never thought about |
| **Phase G — AbortSignal, the cancellation substrate** | Every cancellable API in the engine takes one — `timers/promises`, `events.once`, `events.on`, streams, fs, readline — and a fixture already proved those APIs cancel. Nobody had checked THE SIGNAL. Twenty-eight behaviours; six were wrong, and all six came from the same mistake: ours was shaped like an EventEmitter where node's is an **EventTarget**. The same listener added twice fired twice (EventTarget deduplicates on type and function); `this` inside a listener was the listener itself rather than the signal, which is how a shared handler tells WHICH signal fired when it is attached to several; and `onabort` jumped to the front of the queue instead of taking the place it was assigned in — node runs `listener1, onabort, listener2` for the obvious registration order, we ran `onabort` first. The other three were the REASON, which is the part callers actually branch on: a signal's reason is a `DOMException` carrying the legacy numeric code — 20 for an abort, 23 for a timeout — and ours carried none, so code distinguishing a cancellation from a deadline could not. Fixed with the real code table. `AbortSignal.timeout` is also UNREF'd now: a deadline is not a reason for a process to stay alive, and a CLI that gives every request a two-minute timeout would otherwise refuse to exit for two minutes. That one is asserted by ELAPSED TIME rather than by output — the corpus leaves a 100-second timeout signal pending and the run has to end anyway, which no line of stdout can show |
| **Phase G — web streams, and the bridge that was not there** | A fetch body IS a web stream, and `Readable.toWeb`/`fromWeb` are the only way node code and platform code reach each other. Twenty-six behaviours; **eleven were wrong, and five of them did not exist at all**: every one of the node bridges — `Readable.toWeb`, `Readable.fromWeb`, `Writable.toWeb`, `Writable.fromWeb`, `Duplex.toWeb` — was `undefined`, so the two halves of the runtime could not be connected in either direction. The rest were the rules the platform ENFORCES and ours only implied. A stream is **locked** to one reader: a second `getReader()` must throw, and ours handed out as many as asked, which is how two consumers silently eat each other's chunks. A controller reports `desiredSize` — node counts down 1, 0, 0 as a chunk is queued and the stream closes; ours answered a constant 1, so a producer watching for backpressure never saw any. Enqueueing after close and closing twice are both `TypeError`s and ours accepted them in silence. And **`tee()` was broken in the way that matters**: both branches shared a cursor, so the second branch got `yx` where node gives both branches `xy` — chunks arriving out of order, to a consumer with no way to notice. Rewritten with one read of the source feeding two independent queues, which is also why the original ends up locked. Two things the sweep would not assert: a binary stream may COALESCE two enqueued strings into one Buffer, so the bridge tests content and an object-mode case tests the boundaries where they are real; and `getReader({ mode: 'byob' })` now refuses with a true reason rather than quietly handing back a default reader that ignores the buffer it was given |
| **Phase G — the fetch value types** | `Headers`, `Request`, `Response`, `Blob`, `FormData` — everything an HTTP client written for the platform is built on, and every interesting thing about them is LOCAL. No network, no server, nothing to arrange: a header folds case, a body is readable exactly once, a clone buys a second read. Thirty-two behaviours, seven wrong. **A body could be read as many times as you liked** — the single most load-bearing rule in the whole surface, because two pieces of code both believing they own the payload is exactly the bug `bodyUsed` exists to expose, and in a real request only one of them would have got it. **`response.body` was never null**: a bodyless response handed back an empty stream, so the check that tells a 204 from an empty 200 always said the same thing. The status guards were absent — 204, 205 and 304 are defined bodyless and node refuses a body at construction rather than letting every client drop it silently, and a status outside 200..599 is a `RangeError`. A `GET` with a body was accepted, which a server answers with a puzzle instead of an error. Header names were never validated, so a name with a space in it built a request that cannot be serialised and failed far from the line that caused it. And **`Set-Cookie` iterated joined**: `get` combines values with a comma like any other header, but iteration must yield each cookie separately, because a client that rejoins them cannot split them again — a cookie value may contain a comma. **One refusal was retired rather than fixed.** `Response.clone()` on a still-arriving body used to refuse, on the true-at-the-time grounds that our `ReadableStream` had no `tee`. The previous boundary gave it one, so the refusal had quietly become a false statement; it tees now, and the gate reads both halves. **And the status guard found something nothing else could have.** With it in place, ink stopped rendering — not with an error, but with a program that never settled. The Response being refused was OUR OWN: a `data:` URL fetch, which URLSession answers with no HTTPURLResponse, and which the bridge was reporting as `status: 0`. That is how a wasm module travels inside a bundle, and it is exactly how ink loads its layout engine. node answers a `data:` URL with **200** and the media type from the URL; zero says NO RESPONSE, which reads as a failed fetch. The guard did not break ink — it made a two-year-old lie load-bearing enough to see |
| **Phase G — readline, which every CLI that is not a TUI reads through** | Drivable from a plain stream, so none of it needs a terminal — which is why never having compared the line rules had no excuse. Twenty-four behaviours. **`for await (const line of rl)` threw**: the interface had no async iterator at all, and that is the standard way a program reads stdin a line at a time. It has one now, and lines that arrive while the loop body is busy are QUEUED rather than dropped, which is the only version of it that is safe to use. Breaking out of the loop closes the interface, as node's does. **`rl.question(query, { signal })` never settled** when the signal fired — a prompt with no way out, left waiting on a person who has already gone; the promise form ignored the option entirely. And `rl.cursor` reported 0 on an empty line where node reports `undefined`: zero is a claim about where the cursor is, and there is no line for it to be in. The abort behaviour was checked against node rather than assumed, which was worth doing — readline rejects with node's OWN `AbortError`, discarding whatever reason the caller aborted with, where `timers/promises` rejects with the reason. Two neighbouring APIs, opposite answers; both are now pinned by a scenario that aborts with a custom reason. Everything about where a line ENDS already matched, including the one that usually does not: a `\r` and a `\n` arriving in different writes are one break, not two |
| **Phase G — `performance`, and two numbers that were invented** | The one global nothing had ever checked, and what every build tool and benchmark reaches for. `mark` and `measure` were **empty functions**: a caller could mark and measure all it liked and find nothing to read back, which is the failure where a tool reports no timings and blames its own instrumentation. There was no entry buffer at all, so `getEntries`, `getEntriesByName`, `getEntriesByType`, `clearMarks`, `clearMeasures` and `toJSON` were simply absent. Now: real `PerformanceMark` and `PerformanceMeasure` entries, measure between two marks or from an options object (`{start, end, duration, detail}`), entries sorted by when they STARTED rather than when they were recorded (a measure is recorded after the marks it spans, so recording order is the wrong order), a missing mark raising the DOM's `SyntaxError` as node does rather than measuring from zero and reporting a plausible lie, and a `PerformanceObserver` that batches on a later turn instead of firing inside the `mark()` that produced it. **Two stubs were answering with invented numbers.** `monitorEventLoopDelay` reported `mean: 0` always, and `eventLoopUtilization` did not exist — both are figures an operator uses to decide whether a process is saturated, and a fabricated one is worse than none. The loop now COUNTS the nanoseconds it spends asleep against the ones it spends running callbacks, and utilization is that ratio; the delay monitor parks a timer at a known interval and records how late each firing actually was. Timings cannot be compared between two runs, so the gate asserts the relationships instead — utilization inside 0..1, idle growing across a wait, a sampler that has samples, and a measure spanning a real 40 ms wait that is not zero |
| **Running real work — first night on an actual phone** | The whole of phase G was verified headlessly and never once run on a device. Twelve minutes on the simulator produced three findings no harness could have. **`npm install` succeeds in total silence** — the express tree landed correctly and printed nothing at all, which on mobile data is indistinguishable from a hang. **The hosting path works end to end**: express installed from the registry, served on port 7777 inside the app, and answered `curl` from outside the simulator with `X-Powered-By: Express`. **A running server holds the terminal hostage**, because `&` is still refused until real processes exist — so the dev-server goal needs phase E, not just node. Then `npx n8n` named its own wall: **`package not found: zod-from-json-schema-v3`**, a package nobody ever published. It is an npm ALIAS — `"name": "npm:real-name@range"` — and the resolver had no concept of them (zero occurrences of `npm:` in the file), so it asked the registry for the alias and took the 404 as fatal. Fixed and gated: aliases resolve to the published package and install under the requested name, proven by installing `string-width-cjs` (an alias of string-width@4) beside a direct dependency on string-width@5. Re-running `npx n8n` on the device then travelled far deeper into the tree before stopping — on my own Mac's disk being 100% full, not on anything iOS did |
...
- **A SWEEP for reachable gaps, and brotli fell to it immediately.** Six refusals had
  ...
  **this one was WRONG too and fell on 2026-07-30: the "bignum" was JavaScriptCore's
  native `BigInt`, present all along**),
...
- **`crypto.diffieHellman` — a real constraint attached to the wrong function.** The
  refusal read "finite-field DH needs a bignum implementation", which was believed true
  of `crypto.createDiffieHellman` (and later proved false there as well — see the
  2026-07-30 entry: JSC's native `BigInt` was the bignum, so that half of the refusal
  was imaginary too). But `crypto.diffieHellman({privateKey,
  ...
  about the function it names. X448 refused too, for want of CryptoKit support (SUPERSEDED —
  it is written out over `BigInt` now, see the journal row above) — and the
